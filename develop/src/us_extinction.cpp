...
US_Extinction::US_Extinction(QString temp_projectName, int temp_investigatorID, QWidget *p , const char *name) : QFrame(p, name)
{
   ...
   cnt_order->setRange(1, 50, 1);
   cnt_order->setNumButtons(2);
   ...
   cnt_wavelength->setRange(lambdaLimitLeft, lambdaLimitRight, 1);
   ...
   lbl_lambdaLimitLeft = new QLabel(tr(" Lower Lambda Limit:"), this);
   Q_CHECK_PTR(lbl_lambdaLimitLeft);
   lbl_lambdaLimitLeft->setAlignment(AlignLeft|AlignVCenter);
   lbl_lambdaLimitLeft->setPalette( QPalette(USglobal->global_colors.cg_label, USglobal->global_colors.cg_label, USglobal->global_colors.cg_label));
   lbl_lambdaLimitLeft->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1, QFont::Bold));
   
   le_lambdaLimitLeft = new QLineEdit(this, "lambdaLimitLeft");
   le_lambdaLimitLeft->setPalette(QPalette(USglobal->global_colors.cg_normal, USglobal->global_colors.cg_normal, USglobal->global_colors.cg_normal));
   le_lambdaLimitLeft->setText(str.sprintf(" %2.1f", lambdaLimitLeft));
   connect(le_lambdaLimitLeft, SIGNAL(textChanged(const QString &)), 
           SLOT(update_lambdaLimitLeft(const QString &)));   

   lbl_lambdaLimitRight = new QLabel(tr("Upper Lambda Limit:"), this);
   Q_CHECK_PTR(lbl_lambdaLimitRight);
   lbl_lambdaLimitRight->setAlignment(AlignRight|AlignVCenter);
   lbl_lambdaLimitRight->setPalette( QPalette(USglobal->global_colors.cg_label, USglobal->global_colors.cg_label, USglobal->global_colors.cg_label));
   lbl_lambdaLimitRight->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1, QFont::Bold));
   
   le_lambdaLimitRight = new QLineEdit(this, "lambdaLimitRight");
   le_lambdaLimitRight->setPalette(QPalette(USglobal->global_colors.cg_normal, USglobal->global_colors.cg_normal, USglobal->global_colors.cg_normal));
   le_lambdaLimitRight->setText(str.sprintf(" %2.1f", lambdaLimitRight));
   connect(le_lambdaLimitRight, SIGNAL(textChanged(const QString &)), 
           SLOT(update_lambdaLimitRight(const QString &)));   
   ...
   le_extinction->setText(str.sprintf(" %2.4f", extinction_coefficient));
...
void US_Extinction::setup_GUI()
{
   ...
   j++;
   subGrid1->addWidget(lbl_lambdaLimitLeft,j,0);
   subGrid1->addWidget(le_lambdaLimitLeft,j,1);
   j++;
   subGrid1->addWidget(lbl_lambdaLimitRight,j,0);
   subGrid1->addWidget(le_lambdaLimitRight,j,1);
   j++;
   ...
   background->setColSpacing(0,270);
...
bool US_Extinction::isComment(const QString &str)
{
   ...
   // first replace  quotation marks and commas with spaces, then use simplifyWhiteSpace()
   teststr = teststr.replace("\"", " ");
   teststr = teststr.replace(",", " ");
   teststr = teststr.simplifyWhiteSpace();
   //cout << "teststr: " << teststr << endl;
   str1 = getToken(&teststr, " ");
   //cout << "str1: " << str1 << endl;
   str2 = getToken(&teststr, " ");
   //cout << "str2: " << str2 << endl;
   if (teststr.length() == 0 && str1.toFloat() && str2.toFloat()) // we have wavelength and absorbance
   {
...
bool US_Extinction::loadScan(const QString &fileName)
{
   ...
   if (f.open(IO_ReadOnly | IO_Translate))
   {
      ...
      while (!ts.eof())
      {
         ...
         bool flag1 = true; //true while text information is not a number
         while (flag1 && !ts.eof())
         {
            str1 = ts.readLine();
            if (isComment(str1))
            {
               wls.description += str1;
               str1 = ts.readLine();
            }
            else
            {
	 ...
	 tr("This wavelength scan doesn't have\n"
         "enough usable points - scan not loaded"));
         return (false);
      }
      if (wls.lambda[0] > wls.lambda[wls.lambda.size() - 1])
      {// we need to reverse the order of entries
         WavelengthScan wls2;
         wls2.lambda.clear();
         wls2.od.clear();
	 for (int i=(wls.lambda.size() - (unsigned int) 1); i>=0; i--)
	 {
		 wls2.lambda.push_back(wls.lambda[i]);
		 wls2.od.push_back(wls.od[i]);
		 //cout << i << ": " << wls.lambda[i] << " " << wls.od[i] << endl;
	 }
	 wls.lambda.clear();
	 wls.od.clear();
	 for (unsigned int i=0; i<wls2.lambda.size(); i++)
	 {
		 wls.lambda.push_back(wls2.lambda[i]);
		 wls.od.push_back(wls2.od[i]);
		 //cout << i << "= " << wls.lambda[i] << " " << wls.od[i] << endl;
	 }

      }
      wavelengthScan_vector.push_back(wls);
      str1.sprintf(tr("Scan %d: "), wavelengthScan_vector.size());
      str1 += wls.filename + ", ";
...
void US_Extinction::update_plot()
{
   WavelengthScan wls;
   wls.lambda.clear();
   wls.od.clear();
   bool flag1 = false;
   if (selected_wavelength > lambdaLimitRight)
   {
      selected_wavelength = lambdaLimitRight;
      flag1 = true;
   }
   if (selected_wavelength < lambdaLimitLeft)
   {
      selected_wavelength = lambdaLimitLeft;
      flag1 = true;
   }
   if (flag1)
   {
      // mention to the user that the extinction coefficient
      // calibration wavelength has been adjusted...
      QMessageBox::message(tr("UltraScan Notice:"),
      tr("PLEASE NOTE:\n"
	 "The wavelength selected for the extinction\n"
         "coefficient calibration has been adjusted\n"
	 "to be within the range of the currently\n"
	 "selected wavelength limits"));
      
   }
   for (unsigned int i=0; i<wavelengthScan_vector.size(); i++)
   {
      for (unsigned int j=0; j<wavelengthScan_vector[i].lambda.size(); j++)
      {
         if (wavelengthScan_vector[i].lambda[j] >= lambdaLimitLeft && 
             wavelengthScan_vector[i].lambda[j] <= lambdaLimitRight &&
	     wavelengthScan_vector[i].od[j] <= odCutoff)
	 {
            wls.lambda.push_back(wavelengthScan_vector[i].lambda[j]);
            wls.od.push_back(wavelengthScan_vector[i].od[j]);
	 }
      }
      wavelengthScan_vector[i].lambda.clear();
      wavelengthScan_vector[i].od.clear();
      for (unsigned int j=0; j<wls.lambda.size(); j++)
      {
         wavelengthScan_vector[i].lambda.push_back(wls.lambda[j]);
         wavelengthScan_vector[i].od.push_back(wls.od[j]);
      }
      wls.lambda.clear();
      wls.od.clear();
   }
...
void US_Extinction::plot()
{
   ...
   for (i=0; i<wavelengthScan_vector.size(); i++)
   {
      ...
      for (j=0; j<wavelengthScan_vector[i].lambda.size(); j++)
      {
         ...
         y_plot[i][j] = wavelengthScan_vector[i].od[j]/factor;
      }
      US_lsfit *polyfit;
      ...
      if (fitted)
      {
         for (j=0; j<wavelengthScan_vector[i].lambda.size(); j++)
         {
            ...
            for (k=0; k<order; k++)
            {
               ...
               - (pow((wavelengthScan_vector[i].lambda[j]
               - fitparameters[wavelengthScan_vector.size() + (3 * k) + 1]), 2)
               / ( 2 * pow(fitparameters[wavelengthScan_vector.size() + (3 * k) + 2], 2))));
            }
            poly_plot[i][j] *= fitparameters[i]/factor; 
         }
      }
...
void US_Extinction::update_lambdaLimitLeft(const QString &str)
{
   lambdaLimitLeft = str.toFloat();
   if (lambdaLimitLeft > lambdaLimitRight) return;
   if (lambda_min < lambdaLimitLeft)
   {
      lambda_min = lambdaLimitLeft; // adjust lambda_min to the cutoff
   }
   if (selected_wavelength < lambdaLimitLeft)
   {
      selected_wavelength = lambdaLimitLeft;
...
void US_Extinction::update_lambdaLimitRight(const QString &str)
{
   lambdaLimitRight = str.toFloat();
   if (lambdaLimitRight < lambdaLimitLeft) return; // when editing this field single digits can occur
   if (lambdaLimitRight > lambdaLimitLeft + 20)
   {
      lambda_max = lambdaLimitRight; // adjust lambda_min to the cutoff
   }
   if (selected_wavelength > lambdaLimitRight)
   {
      selected_wavelength = lambdaLimitRight;
      ...
      QMessageBox::message(tr("UltraScan Warning:"),
                           tr("Please note:\n\n"
                           "The extinction coefficient wavelength was\n"
                           "adjusted to match the upper wavelength limit."));
			   */
   }
}
      ...
      case QMessageBox::Yes:
         {
            ...
            for (unsigned int i=0; i<filenames.size(); i++)
            {
	          ...
	          str1 += fi.fileName();
		  str1 += wavelengthScan_vector[i].description;
                  f.close();
               }
               lb_scans->insertItem(str1);
            }
