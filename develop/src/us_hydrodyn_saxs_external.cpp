...
int US_Hydrodyn_Saxs::run_saxs_iq_foxs( QString pdb )
{
   ...
   foxs->addArgument( QDir::convertSeparators( pdb ) );
...
int US_Hydrodyn_Saxs::run_saxs_iq_crysol( QString pdb )
{
        ...
        QFileInfo(crysol_last_pdb).fileName() != QFileInfo(crysol_last_pdb_base).fileName() )
   {
      ...
      cout << QString( "copied file %1 to %2\n" ).arg( f.name() ).arg( f2.name() );
      f.close();
      f2.close();
   ...
   crysol->addArgument( QDir::convertSeparators(
                                                our_saxs_options->crysol_version_26 ? 
                                                QFileInfo(use_pdb).fileName() : QFileInfo( use_pdb ).filePath() ) );
   ...
   editor->append("\n\nStarting Crysol\n");
   cout << crysol->arguments().join(" ") << endl;
   crysol->start();
   external_running = true;
