...
US_ViewMWL::US_ViewMWL(QWidget *p, const char *name) : QFrame(p, name)
{
   ...
   lbl_datadir = new QLabel("",this);
   lbl_datadir->setAlignment(AlignCenter|AlignVCenter);
   lbl_datadir->setFrameStyle(QFrame::WinPanel|Sunken);
   lbl_datadir->setPalette( QPalette(USglobal->global_colors.cg_edit, USglobal->global_colors.cg_edit, USglobal->global_colors.cg_edit) );
   lbl_datadir->setFont(QFont( USglobal->config_list.fontFamily, USglobal->config_list.fontSize - 1));
...
void US_ViewMWL::load()
{
   ...
   if (file_format) //Load ASCII
   {
	 ...
	 QDir d(filename);
         j = filename.findRev("/");
         lbl_datadir->setText(filename.left(j));
...
void US_ViewMWL::setup_GUI()
{
   ...
   j++;
   controlGrid->addMultiCellWidget(lbl_datadir, j, j, 0, 1);
   j++;
